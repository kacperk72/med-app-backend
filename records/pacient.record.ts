...
export class PacientRecord implements PacientEntity {
  ...
  async insert(): Promise<string> {
    ...
    await pool.execute(
      'INSERT INTO `dane_logowania`(`user_id`,`login`,`haslo`,`rola`,`imie`,`nazwisko`) VALUES(:id, :login, :haslo, :rola, :imie, :nazwisko)',
      {
        id: this.id,
        login: this.login,
        haslo: this.password,
    ...
    if (this.role == 'pacjent') {
      await pool.execute(
        'INSERT INTO `pacjenci`(`id_pacjenta`, `wiek`) VALUES (:id, :wiek)',
        {
          id: this.id,
          wiek: this.age,
        }
  ...
  static async listAll(): Promise<PacientRecord[]> {
    const [results] = (await pool.execute(
      'SELECT * FROM `pacjenci` LEFT JOIN `dane_logowania` ON pacjenci.id_pacjenta = dane_logowania.user_id;'
    )) as PacientRecordResult;
  ...
  static async getUser(login: string): Promise<Object | null> {
    ...
    const [result] = await pool.execute<RowDataPacket[]>(
      'SELECT `user_id`,`name`,`surname` FROM `dane_logowania` WHERE login = :login',
      {
        login,
      }
    );
  ...
  static async getVisits(user_id: string): Promise<Object | null> {
    const [result] = await pool.execute<RowDataPacket[]>(
      'SELECT * FROM `wizyty` WHERE `id_pacjenta` = :user_id',
      {
        user_id,
      }
    );
  ...
  static async getName(id_lekarza: string) {
    const [result] = await pool.execute<RowDataPacket[]>(
      'SELECT `name`,`surname` FROM `dane_logowania` WHERE `user_id` = :id_lekarza',
      {
        id_lekarza,
      }
    );
  ...
  static async getSpec(id_lekarza: string) {
    const [result] = await pool.execute<RowDataPacket[]>(
      'SELECT `speciality` FROM `lekarze` WHERE `id_lekarza` = :id_lekarza',
      {
        id_lekarza,
      }
    );
  ...
  static async getCities() {
    const [result] = await pool.execute(
      'SELECT DISTINCT `city` FROM `lekarze`'
    );
  ...
  static async getSpecialities() {
    const [result] = await pool.execute(
      'SELECT DISTINCT `speciality` FROM `lekarze`'
    );
